...
class Admin {
	...
	private function dependencies() {
		...
		if ( bsacc_acf_pro() ) {
			include_once BSACC_PATH . 'admin/class-settings-fields-site-acf.php';
		}
		...
		if ( ( bsacc_classicpress() || bsacc_new_cms() ) && $editor ) {
			include_once BSACC_PATH . 'admin/classic-editor/classic-editor.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once BSACC_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once BSACC_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once BSACC_PATH . 'admin/class-admin-toolbar-menus.php';

		// Functions for various admin pages and edit screens.
		require_once BSACC_PATH . 'admin/class-admin-pages.php';

		// Import custom fields for editing, if ACF Pro is active.
		if ( bsacc_acf_options() ) {
			include_once BSACC_PATH . 'admin/class-fields-import.php';
		}

		// Filter by page template.
		require_once BSACC_PATH . 'admin/class-admin-template-filter.php';
	...
	public function admin_footer() {
		...
		if ( bsacc_classicpress() ) {
			$default = __( 'website powered by ClassicPress', 'bsac-cabins' );

		// If the site is running WordPress.
		} else {
			$default = __( 'website powered by WordPress', 'bsac-cabins' );
		}
		...
		if ( bsacc_acf_options() ) {
			 ...
			 */
			$credit = get_field( 'bsacc_admin_footer_credit', 'option' );
			$link   = get_field( 'bsacc_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'bsac-cabins' ),
					esc_url( $link ),
					$credit
				);
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'bsacc_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'bsacc_footer_link' ) );
		...
		$admin_footer = apply_filters( 'bsacc_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( BSACC_ADMIN_SLUG . '-tooltips', BSACC_URL . 'admin/assets/css/tooltips.min.css', [], BSACC_VERSION, 'all' );
		 ...
		 */
		if ( bsacc_acf() ) {
			wp_enqueue_style( BSACC_ADMIN_SLUG . '-acf', BSACC_URL . 'admin/assets/css/acf.css', [], BSACC_VERSION, 'all' );
		}
		 ...
		 * @since 1.0.0
		 */
		$welcome = get_option( 'bsacc_custom_welcome' );
		if ( $welcome ) {
			wp_enqueue_style( BSACC_ADMIN_SLUG . '-welcome', BSACC_URL . 'admin/assets/css/welcome.css', [], BSACC_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require BSACC_PATH . 'admin/partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( BSACC_ADMIN_SLUG . '-conditionalize', BSACC_URL . 'admin/assets/js/admin.js', [ 'jquery' ], BSACC_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( BSACC_ADMIN_SLUG . '-admin', BSACC_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], BSACC_VERSION, true );
