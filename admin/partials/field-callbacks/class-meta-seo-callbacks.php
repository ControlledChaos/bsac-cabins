...
class Meta_SEO_Callbacks {
	...
	public function disable_meta( $args ) {
		...
		$option = get_option( 'bsacc_meta_disable' );

		$html = '<p><input type="checkbox" id="bsacc_meta_disable" name="bsacc_meta_disable" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="bsacc_meta_disable"> '  . $args[0] . '</label></p>';
	...
	public function  schema_org_type( $args ) {
		...
		// Get the field file.
		include_once BSACC_PATH . 'admin/partials/field-callbacks/schema-org-type.php';
 ...
 */
function bsacc_meta_seo_callbacks() {
