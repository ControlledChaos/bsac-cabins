...
// namespace BSAC_Cabins\Includes\Tools\Database_Reset;
...
<div class="wrap">
	<h2><?php _e( 'Database Reset', 'bsac-cabins' ) ?></h2>
	<p class="description"><?php _e( 'Reset the WordPress database back to its original state.', 'bsac-cabins' ); ?></p>
	<hr />

	<?php include( 'partials/notice.php' ) ?>

	<form method="post" id="db-reset-form">
		<p>1. <?php _e( 'Select the database tables you would like to reset', 'bsac-cabins' ) ?>:</p>

		<div id="select-container">
			<a href='#' id="select-all"><?php _e( 'Select All', 'bsac-cabins' ) ?></a>
		...
		<p id="reactivate" style="display: none">
			...
			<?php _e( 'Reactivate current theme and plugins after reset?', 'bsac-cabins' ) ?>
			</label>
		</p>

		<p id="disclaimer" style="display: none">
			<em><?php printf( __( 'You selected the users table. Only the <strong><u>%s</u></strong> user will be restored', 'bsac-cabins' ), $this->user->user_login ) ?>.</em>
		</p>
		<p>
		2. <?php _e( 'Enter the security code into the text box', 'bsac-cabins' ) ?>:
		...
		<p>
			<input type="submit" name="db-reset-submit" value="<?php _e( 'Reset Tables', 'bsac-cabins' ) ?>" id="db-reset-submit" class="button-primary" disabled />
