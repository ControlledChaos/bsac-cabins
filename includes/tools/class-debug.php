...
if ( ! defined( 'DOING_AJAX' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
    add_action( 'admin_init', 'bsacc_check_wp_debug_define' );
}

function bsacc_check_wp_debug_define() {
	if ( defined( 'WP_DEBUG' ) && ! WP_DEBUG ) {
		$dev_mode = get_option( 'bsacc_site_development' );
		if ( $dev_mode ) {
			bsacc_set_wp_debug_define( true );
		} elseif ( false == $dev_mode )  {
			bsacc_set_wp_debug_define( false );
		}
	}
}
 ...
 * @return void
 */
function bsacc_set_wp_debug_define( $turn_it_on ) {
	...
	$config_file_path = bsacc_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
    }
	...
	*/
	apply_filters( 'bsacc_set_wp_debug_define', $turn_it_on );
 ...
 */
function bsacc_find_wpconfig_path() {
 ...
 * @return string|bool The path of wp-config.php file or false
 */
register_deactivation_hook( __FILE__, 'bsacc_deactivation' );

function bsacc_deactivation() {
    // set WP_DEBUG back to false
    set_bsacc_wp_debug_off( false );
}

function set_bsacc_wp_debug_off( $turn_it_off ) {
	// Get path of the config file
	$config_file_path = bsacc_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
	 ...
	 * @param string $turn_it_on The value of WP_DEBUG constant
	*/
	apply_filters( 'bsacc_set_wp_debug_define', $turn_it_off );
