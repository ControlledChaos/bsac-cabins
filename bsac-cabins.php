 ...
 * Description:  Cabins plugin for Big Santa Anita Canyon sites.
 * Version:      1.0.0
 ...
 */
function bsacc_about_link( $links ) {
	...
	if ( is_admin() ) {
		...
		if ( true == $position ) {
			$url = admin_url( 'index.php?page=' . BSACC_ADMIN_SLUG . '-settings' );
		} else {
			$url = admin_url( 'admin.php?page=' . BSACC_ADMIN_SLUG . '-settings' );
		}

		// Create new settings link array as a variable.
		$about_page = [
			sprintf(
				'<a href="%1s" class="' . BSACC_ADMIN_SLUG . '-page-link">%2s</a>',
				admin_url( 'plugins.php?page=' . BSACC_ADMIN_SLUG . '-page' ),
				esc_attr( 'Documentation', 'bsac-cabins' )
			),
		];
...
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'bsacc_about_link' );
 ...
 * @link   https://codex.wordpress.org/Plugin_API/Filter_Reference/plugin_action_links_(plugin_file_name)
 */
function bsacc_settings_links( $links, $file ) {
	...
	if ( is_admin() ) {
		...
		if ( bsacc_acf_options() ) {
			...
			// Get the field.
			$acf_position = get_field( 'bsacc_settings_link_position', 'option' );
		...
		if ( $file == plugin_basename( __FILE__ ) ) {
			...
			$links[] = sprintf(
				'<a href="%1s" class="' . BSACC_ADMIN_SLUG . '-settings-link">%2s</a>',
				$url,
				esc_attr( 'Site Settings', 'bsac-cabins' )
			);
			$links[] = sprintf(
				'<a href="%1s" class="' . BSACC_ADMIN_SLUG . '-scripts-link">%2s</a>',
				admin_url( 'options-general.php?page=' . BSACC_ADMIN_SLUG . '-scripts' ),
				esc_attr( 'Script Options', 'bsac-cabins' )
			);

			// Add a placeholder for an upgrade link.
			$links[] = sprintf(
				'<a href="%1s" title="%2s" class="' . BSACC_ADMIN_SLUG . '-upgrade-link" style="color: #888; cursor: default;">%3s</a>',
				''/* Add upgrade URL here */,
				__( 'Upgrade not available', 'bsac-cabins' ),
				esc_attr( 'Upgrade', 'bsac-cabins' )
			);
...
add_filter( 'plugin_row_meta', 'bsacc_settings_links', 10, 2 );
